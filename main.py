...
def detectar_outliers(columna):
    ...
    outliers = columna[(columna > lim_sup) | (columna < lim_inf)]
...
for columna in datos_limpios.columns:
    ...
    lim_sup = media + 3 * desviacion
    lim_inf = media - 3 * desviacion
    datos_limpios = datos_limpios[(datos_limpios[columna] <= lim_sup) & (datos_limpios[columna] >= lim_inf)]

print("Datos originales:", len(datos_leidos))
print("Datos limpios:", len(datos_limpios))

# GUARDAR DATASET LIMPIO
datos_limpios.to_csv("dataset_limpio.csv",index=False)
...
with open("reporte.txt", "w") as reporte:
    ...
    reporte.write(f"Cantidad datos originales: {len(datos_leidos)}\n")
    reporte.write(f"Cantidad datos limpios: {len(datos_limpios)}\n")
